...
function getPathWithQueryStringUseUnescapeParams({
  ...
  path = (event.pathParameters && event.pathParameters.proxy && `/${event.pathParameters.proxy}`) ||
    event.path,
  ...
  if (event.multiValueQueryStringParameters) {
    for (const key in event.multiValueQueryStringParameters) {
      ...
      query[formattedKey] = event.multiValueQueryStringParameters[key]!.map((value) =>
...
const getResponseToAlb: EventSource<ALBEvent, ALBResult>["getResponse"] = ({
  ...
  const multiValueHeaders = !event.headers ? getMultiValueHeaders(responseHeaders) : undefined;
