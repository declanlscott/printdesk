import {
  getSignedCookies as awsGetSignedCookies,
  getSignedUrl as awsGetSignedUrl,
...
interface CloudFrontSigner$ {
   ...
   */
  getSignedCookies(input: GetSignedCookiesInput): Effect.Effect<CloudfrontSignedCookiesOutput>;
}

const makeCloudFrontSigner = (credentials: CloudFrontSignerCredentials): CloudFrontSigner$ => {
 ...
 * @category models
 */
export class CloudFrontSigner extends Context.Service<CloudFrontSigner, CloudFrontSigner$>()(
   ...
   * );
   */
  public static readonly layer = (config: CloudFrontSigner.Config) =>
    Layer.succeed(this, makeCloudFrontSigner(config));
}
