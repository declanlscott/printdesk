...
import { WorkflowStatusesReadRepository, WorkflowStatusesWriteRepository } from ".";
import {
...
export const makeReadRepository = Effect.gen(function* () {
  const base = yield* readRepositoryFactory(WorkflowStatusesContract.Table);
...
export const readRepositoryLayer = makeReadRepository.pipe(
  Layer.effect(WorkflowStatusesReadRepository),
);

export type WriteRepository = Effect.Success<typeof makeWriteRepository>;
export const makeWriteRepository = WorkflowStatusesReadRepository.pipe(
  Effect.flatMap((repository) =>
    writeRepositoryFactory(WorkflowStatusesContract.Table, repository),
  ),
);
export const writeRepositoryLayer = makeWriteRepository.pipe(
  Layer.effect(WorkflowStatusesWriteRepository),
);
