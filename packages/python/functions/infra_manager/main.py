...
from powertools import logger, processor, tracer
...
def record_handler(record: InputDynamoDBStreamRecord):
    ...
    callback_id = None if is_destroy else record.dynamodb.NewImage.callback_id

    with callback.heartbeat(callback_id=callback_id) if callback_id else nullcontext():
        ...
        stack = pulumi.automation.create_or_select_stack(
            ...
            program=lambda: inline(
                tenant_id=tenant_id,
                _input=record.dynamodb.OldImage
                if is_destroy
                else record.dynamodb.NewImage,
            ),
        ...
        pulumi.runtime.register_stack_transformation(
            naming.transform_resource(tenant_id)
        )
        ...
        if not is_destroy:
            try:
                ...
                if callback_id:
                    callback.resolve_success(
                        callback_id=callback_id,
                        result_json=result.outputs["output"].value,
                    )
