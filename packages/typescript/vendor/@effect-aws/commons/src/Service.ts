 ...
 * @category models
 */
export type CommandCtor<I> = new (
  input: I,
  ...args: Array<any>
) => CommandImpl<I, any, BaseResolvedConfig>;

/**
 * @since 0.4.0
 * @category models
 */
export type PaginatorCtor<I> = (
...
export const catchServiceExceptions =
  (errorTags?: NonEmptyReadonlyArray<string>) => (e: unknown) => {
...
export const makeServiceStreamFn = (
  ...
  return (args: any, options?: HttpHandlerOptions) => {
    ...
    return Stream.fromAsyncIterable(paginator, catchServiceExceptions(fnOptions.errorTags));
  };
};
    ...
    const effectCommands = pipe(
      ...
      Record.mapEntries((CommandCtor, command) => {
        const ExtendedCommand = class extends CommandCtor {
          ...
          public override resolveMiddleware(
