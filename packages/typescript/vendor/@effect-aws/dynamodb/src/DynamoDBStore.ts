...
type WithoutTableNameInArgs = {
  ...
  >[0] extends { TableName?: string }
    ? (
        ...
        Effect.Success<ReturnType<DynamoDBDocumentService.Type[K]>> extends { Item?: infer Item }
          ? Item | undefined
          : Effect.Success<ReturnType<DynamoDBDocumentService.Type[K]>> extends {
                Items?: infer Items;
              }
            ? Items
            : Effect.Success<ReturnType<DynamoDBDocumentService.Type[K]>> extends {
  ...
  return {
    ...
    query: (args) =>
      ddb
        ...
        .pipe(Effect.map(({ Items }) => Items ?? [])),
    get: (args) =>
      ddb.get({ TableName: props.tableName, Key: args?.Key }).pipe(Effect.map(({ Item }) => Item)),
    put: (args) =>
      ddb
 ...
 * @category models
 */
export class DynamoDBStore extends Context.Service<DynamoDBStore, DynamoDBStore.Type>()(
  "@effect-aws/dynamodb/DynamoDBStore",
) {
  public static layer = (props: DynamoDBStore.Config) =>
    Layer.effect(this, makeDynamoDBStore(props));
  public static layerConfig = (config: Config.Wrap<DynamoDBStore.Config>) =>
