 ...
 */
import { createLocalJWKSet, errors, jwtVerify, decodeJwt } from "jose";
...
export interface VerifyResult<T extends SubjectSchema> {
   ...
   */
  subject: {
    [type in keyof T]: { type: type; properties: StandardTypedV1.InferOutput<T[type]> };
  }[keyof T];
}
...
export interface Client {
  /**
   * Start the authorization flow. For example, in SSR sites.
   *
   * ```ts
    ...
    async verify<T extends SubjectSchema>(
      ...
      try {
        const result = await jwtVerify<{
          mode: "access";
          type: keyof T;
          properties: StandardTypedV1.InferInput<T[keyof T]>;
        }>(token, jwks, {
          issuer,
        });
