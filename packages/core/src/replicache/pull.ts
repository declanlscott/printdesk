...
export namespace ReplicachePuller {
  /**
          ...
          () =>
            Effect.gen(function* () {
              ...
              yield* AccessControl.userPolicy(
                {
                  name: ReplicacheClientGroupsModel.tableName,
                  id: clientGroupId,
                },
              ...
              onSome: (result) => {
                ...
                const patch = result.clientView.previous.pipe(
                  Option.match({
                    onNone: () =>
                      result.patch.pipe(
                        Chunk.prepend(new ReplicacheContract.ClearOperation()),
                      ),
                    onSome: () => result.patch,
                  }),
                );
        ...
        Effect.gen(function* () {
          ...
          return yield* process(
            ...
            Effect.catchTag("DifferenceLimitExceededError", () =>
              ...
              ).pipe(Effect.andThen(process(pullRequest.clientGroupId, null))),
            ),
          );
        }).pipe(
          Effect.timed,
  ...
  export const layer = Layer.effect(ReplicacheContract.Puller, impl).pipe(
    ...
    Layer.provide(Replicache.ClientViewsRepository.Default),
    Layer.provide(Replicache.ClientViewEntriesRepository.Default),
    Layer.provide(Queries.Differentiator.Default),
  );
}
