...
/**
 * The `issuer` create an OpenAuth server, a [Hono](https://hono.dev) app that's
...
import { encryptionKeys, legacySigningKeys, signingKeys } from "./keys";
...
> {
  ...
  ttl?: {
    ...
    refresh?: number;
    /**
     * Interval in seconds where refresh token reuse is allowed. This helps mitigate
     * concurrency issues.
     * @default 60s
     */
    reuse?: number;
...
>(input: IssuerInput<Providers, Subjects, Result>) {
  ...
  if (process.env.OPENAUTH_STORAGE) {
    const parsed: any = JSON.parse(process.env.OPENAUTH_STORAGE);
  ...
  app.get("/userinfo", async (c) => {
    ...
    const result = await jwtVerify<{
      ...
      properties: StandardTypedV1.InferInput<SubjectSchema[keyof SubjectSchema]>;
