  ...
  if (Predicate.isFunction(handlerOrOptions)) {
    return async (event, context) =>
      handlerOrOptions(event, context).pipe(
        Effect.runPromise as <E>(effect: Effect.Effect<A, E, R>) => Promise<A>,
      );
  }

  const runtime = LambdaRuntime.fromLayer(handlerOrOptions.layer, {
    memoMap: handlerOrOptions.memoMap,
  });
  return async (event, context) => {
...
export const stream: {
  ...
  return global.awslambda?.streamifyResponse(async (event, responseStream, context) => {
    ...
    return handlerOrOptions
      ...
      .pipe(pipeTo(responseStream, { end: true }), runtime.runPromise);
  });
};
...
const requestFromEvent = (event: LambdaHandler.Event): Request => {
  ...
  return new Request(`https://${requestValues.remoteAddress}${requestValues.path}`, {
